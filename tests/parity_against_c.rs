...
#[test]
fn ffi_matches_expected_baseline() {
    let expected = parity_expected::expected_map();

    let era_rust = unsafe { novas::era(2451545.0, 0.0) };
    parity_expected::assert_close("era", era_rust, expected["era"], 1e-13);

    let ee_ct_rust = unsafe { novas::ee_ct(2451545.0, 0.0, 0) };
    parity_expected::assert_close("ee_ct", ee_ct_rust, expected["ee_ct"], 1e-13);
